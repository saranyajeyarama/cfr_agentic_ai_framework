...
import { AlertTriangle, Check, CheckCircle2, ChevronRight, CornerDownRight, X, Info, ShieldAlert, Bot } from 'lucide-react';
...
const PURCHASE_ORDERS: PO[] = mockData.purchaseOrders as PO[];
...
export function OrderTriage() {
  ...
  const [decisionLog, setDecisionLog] = useState<DecisionEntry[]>(mockData.decisionCaptureLog);
  ...
  const handleExecuteDecision = () => {
    const newDecision: DecisionEntry = {
      ...
      agentRecommendation: activePo.recommendedAction, // use recommendedAction
      userDecision: decision || 'reject',
      overrideReason: (decision === 'override' || decision === 'modify' || decision === 'reject') ? overrideReason : null,
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-4">
                  <div className="bg-white rounded-lg p-3 border border-[#DB033B] flex-1 shadow-sm">
                    ...
                    <div className="font-mono text-slate-800 font-bold text-lg">{activePo.proposedAllocation}</div>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-[#DB033B] flex-1 shadow-sm">
                    <div className="text-[10px] text-slate-500 mb-1 uppercase tracking-wider font-bold">Hold / Short</div>
                    <div className="font-mono text-slate-600 font-bold text-lg">{activePo.proposedHold}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              ...
              <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl animate-in slide-in-from-top-2 opacity-100 duration-300 shadow-inner">
                ...
                <div className="mt-5 flex justify-end">
                  <button 
                    ...
                    Execute Override & Log Telemetry
                  </button>
                </div>
              </div>
            )}
            {decision === 'accept' && (
               <div className="flex justify-end animate-in fade-in duration-300">
                  <button 
                    onClick={handleExecuteDecision}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8 py-2.5 rounded-lg text-sm transition-colors shadow-md focus:ring-4 focus:ring-emerald-500/20">
                    Confirm Allocation
                  </button>
               </div>
            )}
...
function QueueItem({ po, active, onClick }: { po: PO; active: boolean; onClick: () => void; key?: string | number }) {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        {isCritical && <div className="w-2 h-2 rounded-full bg-[#DB033B] animate-pulse mt-1"></div>}
        {isWarning && <div className="w-2 h-2 rounded-full bg-amber-500 mt-1"></div>}
      </div>
