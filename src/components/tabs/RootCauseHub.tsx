import { BarChart2, Mail, MessageSquare, Send, ArrowLeft } from 'lucide-react';
...
const DRIVERS = mockData.rootCauseSummary.drivers.reduce((acc, d) => {
...
const L1_DATA: ChartDataPoint[] = [
  ...
  { name: 'Demand Gaps', transparentVal: BASELINE - mockData.rootCauseSummary.demandDrivenCases, visibleVal: mockData.rootCauseSummary.demandDrivenCases, displayVal: `-${mockData.rootCauseSummary.demandDrivenCases.toLocaleString()}`, fill: '#3b82f6', type: 'gap-demand' },
  { name: 'Supply Gaps', transparentVal: BASELINE - mockData.rootCauseSummary.demandDrivenCases - mockData.rootCauseSummary.supplyDrivenCases, visibleVal: mockData.rootCauseSummary.supplyDrivenCases, displayVal: `-${mockData.rootCauseSummary.supplyDrivenCases.toLocaleString()}`, fill: '#eab308', type: 'gap-supply' },
  { name: 'Actual Cases', transparentVal: 0, visibleVal: BASELINE - mockData.rootCauseSummary.totalCasesMissed, displayVal: (BASELINE - mockData.rootCauseSummary.totalCasesMissed).toLocaleString(), fill: '#475569', type: 'actual', isTotal: true }
];
...
const L2_DEMAND_DATA: ChartDataPoint[] = [
  ...
  ...mockData.rootCauseSummary.drivers.filter(d => d.category === 'Demand').map((d, index, arr) => {
     // Compute cumulative sum before this item
     const prevMissed = arr.slice(0, index).reduce((sum, item) => sum + item.casesMissed, 0);
     const transparentVal = BASELINE - prevMissed - d.casesMissed;
...
const L2_SUPPLY_DATA: ChartDataPoint[] = [
  { name: 'Target Cases', transparentVal: 0, visibleVal: BASELINE, displayVal: BASELINE.toLocaleString(), fill: '#475569', type: 'target', disabled: true, isTotal: true },
  ...mockData.rootCauseSummary.drivers.filter(d => d.category === 'Supply').map((d, index, arr) => {
     // Compute cumulative sum before this item
     const prevMissed = arr.slice(0, index).reduce((sum, item) => sum + item.casesMissed, 0);
     const transparentVal = BASELINE - prevMissed - d.casesMissed;
...
export function RootCauseHub() {
  ...
  const [selectedDriverId, setSelectedDriverId] = useState<string>(mockData.rootCauseSummary.drivers[0].id);
  const [draftText, setDraftText] = useState<string>(DRIVERS[mockData.rootCauseSummary.drivers[0].id].draft);
  ...
  const handleBarClick = (data: any) => {
    if (viewLevel === 'L1') {
      if (data.type === 'gap-demand') {
        ...
        const firstDemand = mockData.rootCauseSummary.drivers.find(d => d.category === 'Demand');
        if (firstDemand) {
          setSelectedDriverId(firstDemand.id);
          setDraftText(DRIVERS[firstDemand.id].draft);
        }
      } else if (data.type === 'gap-supply') {
        setViewLevel('L2_SUPPLY');
        const firstSupply = mockData.rootCauseSummary.drivers.find(d => d.category === 'Supply');
        if (firstSupply) {
          setSelectedDriverId(firstSupply.id);
          setDraftText(DRIVERS[firstSupply.id].draft);
        }
      }
    } else {
      if (data.type === 'driver' && data.id) {
        setSelectedDriverId(data.id);
        setDraftText(DRIVERS[data.id].draft);
      }
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
           <div className="flex justify-between items-start mb-6">
             <div>
               ...
               <p className="text-sm font-bold text-slate-800 mt-2">Total CFR Cuts: {mockData.rootCauseSummary.totalCasesMissed.toLocaleString()} Cases (CFR {mockData.rootCauseSummary.cfRActual}% vs Target {mockData.rootCauseSummary.cfrTarget}%)</p>
             </div>
             {viewLevel === 'L1' && (
               <div className="flex gap-4">
                  <button onClick={() => handleBarClick({ type: 'gap-demand' })} className="text-right text-left cursor-pointer hover:bg-slate-50 p-2 -my-2 rounded-lg transition-colors border border-transparent hover:border-slate-200 group">
                    ...
                    <div className="text-amber-700 font-mono font-bold mt-1 shadow-sm">-{mockData.rootCauseSummary.demandDrivenCases.toLocaleString()} CS</div>
                  </button>
                  <button onClick={() => handleBarClick({ type: 'gap-supply' })} className="text-right text-left cursor-pointer hover:bg-slate-50 p-2 -my-2 rounded-lg transition-colors border border-transparent hover:border-slate-200 group">
                    ...
                    <div className="text-[#DB033B] font-mono font-bold mt-1 shadow-sm">-{mockData.rootCauseSummary.supplyDrivenCases.toLocaleString()} CS</div>
                  </button>
               </div>
             )}
             {viewLevel === 'L2_DEMAND' && (
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 flex flex-col shadow-sm">
            ...
            <textarea 
              className="w-full flex-1 bg-slate-50 border border-slate-200 rounded-lg p-4 text-sm text-slate-700 resize-none outline-none focus:border-[#DB033B] focus:ring-1 focus:ring-[#DB033B] transition-all mb-4 shadow-inner leading-relaxed"
              value={draftText}
            ...
            <div className="flex gap-3 mt-auto relative">
              <a 
                href={`mailto:${selectedDriver.ownerName.toLowerCase().replace(/\s+/g, '.')}@company.com?subject=${encodeURIComponent("CFR Root Cause Follow-Up - " + selectedDriver.name)}&body=${encodeURIComponent(draftText)}`}
                className="flex-1 bg-[#DB033B] hover:opacity-90 text-white font-bold py-3 px-4 rounded-lg text-sm flex items-center justify-center gap-2 transition-opacity shadow-sm focus:ring-4 focus:ring-[#DB033B]/20"
              >
