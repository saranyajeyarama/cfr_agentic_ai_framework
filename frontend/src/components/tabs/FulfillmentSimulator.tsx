import { Activity, AlertTriangle, ArrowRight, ShieldAlert, ChevronRight, CheckCircle2, Loader2 } from 'lucide-react';
...
type Incident = {
  ...
  // Real tiger_semantic identifiers — supplied by /dashboard-data so the
  // agent session evaluates the real order. The display `customer` name
  // is NOT a valid identifier. orderedQty defaults when not supplied.
  soldTo?: string;
...
export function FulfillmentSimulator({ data }: { data: DashboardData }) {
  const INCIDENTS: Incident[] = data.fulfillmentIncidents as Incident[];
  ...
  const handleExecute = async () => {
    if (!selectedScenario || isExecuting) return;
    setIsExecuting(true);
    setExecuteResult(null);
    try {
      // Real v2.02 contract. Identifiers come from /dashboard-data; on
      // mock data they are absent and the backend resolves a demo
      // scenario. trigger_type 'manual' — a planner-initiated run.
